        ...
        <div className='space-y-4'>
            {cart?.items?.map((item)=> (
                <div key={item?.book?._id} className='flex items-center gap-4 border rounded-lg p-4 shadow-sm'>
                    <img src={item?.book?.coverImage?.startsWith('http') ? item.book.coverImage : buildApiUrl(`/images/${item?.book?.coverImage?.replace(/^\/+/, '') || 'fallback'}`)}
                     alt={item?.book?.title || "Book cover"} className='rounded w-24 h-32 object-cover'/>
                 ...
                 <div className='flex items-center gap-2 mt-2'>
                    <button className='disabled:opacity-50'   disabled={item?.quantity <= 1} 
                    onClick={()=> updateCart(item?.book._id, item?.quantity - 1)}>-</button>
